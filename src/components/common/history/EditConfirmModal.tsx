// ============================================================
// F.Y.T - EDIT CONFIRM MODAL
// src/components/common/history/EditConfirmModal.tsx
// Modal de confirmation de modification partagée Desktop/Mobile
...
import { Edit2, Calendar, Download, X } from 'lucide-react';
...
interface Props {
  ...
  onConfirm: () => void;
  onCancel: () => void;
  /** True si la séance a été importée manuellement */
    ...
    <>
      ...
      <div className={`flex items-center ${showCloseButton && !anchorRef ? 'justify-between' : 'gap-3'} mb-4 px-5 pt-5`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-theme">Modifier la séance ?</h3>
            <p className="text-sm text-theme-muted">
              {isManualSession ? 'Modifier cette séance importée' : 'Modifier la date de cette séance'}
            </p>
          </div>
        </div>
      ...
      <div className="bg-theme-tertiary/50 rounded-xl p-4 mb-6 border border-theme/50 mx-5">
        ...
        {isManualSession && (
          <div className="mt-2 flex items-center gap-2 text-xs text-[var(--color-primary)]">
            <Download className="w-3 h-3" />
            Séance importée manuellement
          </div>
        )}
      </div>
      ...
      <div className="flex gap-3 px-5 pb-5">
        ...
        <button
          onClick={onConfirm}
          className="flex-1 py-3 bg-[var(--color-primary)] hover:opacity-90 text-white rounded-xl font-medium transition-colors"
        >
          Modifier
        </button>
      </div>
