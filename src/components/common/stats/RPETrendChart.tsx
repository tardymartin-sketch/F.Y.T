...
export const RPETrendChart: React.FC<Props> = ({ userId, weeksBack = 12, onViewDate }) => {
  ...
  if (data.length === 0 || !stats) {
    return (
      <div className="bg-theme-secondary/50 border border-theme rounded-xl p-8 text-center">
        <Activity className="w-12 h-12 text-theme-muted mx-auto mb-3" />
        <p className="text-theme-muted">Aucune donnée RPE disponible</p>
        <p className="text-theme-muted text-sm mt-1">Renseigne le RPE de tes séances</p>
      </div>
    );
  }
  ...
  return (
    <div
      className="bg-theme-secondary/50 border border-theme rounded-xl p-4"
      ref={containerRef}
    ...
    >
      ...
      {/* Stats summary */}
      <div className="mt-4 pt-3 border-t border-theme grid grid-cols-3 gap-4 text-sm text-center">
        <div>
          <span className="text-theme-muted block">Min</span>
          <span className="text-theme font-medium">{stats.minRpe.toFixed(1)}</span>
        </div>
        <div>
          <span className="text-theme-muted block">Moyenne</span>
          <span className="text-amber-400 font-medium">{stats.avgRpe.toFixed(1)}</span>
        </div>
        <div>
          <span className="text-theme-muted block">Max</span>
          <span className="text-theme font-medium">{stats.maxRpe.toFixed(1)}</span>
        </div>
      </div>
    </div>
  );
