...
export interface TooltipData {
  ...
  primaryColor?: string; // Couleur Tailwind (ex: "text-[var(--color-success)]")
...
interface Props {
  ...
  /** Si true, affiche le tooltip en dessous du point par défaut (au lieu d'au-dessus) */
  preferBelow?: boolean;
}
...
export const ChartTooltipOverlay: React.FC<Props> = ({
  ...
  useLayoutEffect(() => {
    ...
    // Style par défaut selon preferBelow
    let left = position.x;
    let top: number;
    let transformX = '-50%';
    ...
    if (preferBelow) {
      // Afficher en dessous du point par défaut
      top = position.y + 15;
      transformY = '0%';
    } else {
      // Afficher au-dessus du point par défaut
      top = position.y - 10;
      transformY = '-100%';
    }
    ...
    if (tooltipRef.current) {
      ...
      // Ajustement vertical selon le mode
      if (preferBelow) {
        // Si déborde en bas quand affiché en dessous, passer au-dessus
        if (top + tooltipHeight > containerHeight - padding) {
          top = position.y - 10;
          transformY = '-100%';
        }
      } else {
        // Si déborde en haut, afficher en dessous
  ...
  }, [position, position?.x, position?.y, position?.containerWidth, position?.containerHeight, preferBelow]);
  ...
  const content = (
    <div className="min-w-[140px] max-w-[200px]">
      ...
      {data.primaryValue && (
        <div className={`font-bold text-base ${data.primaryColor || 'text-theme'}`}>
          {data.primaryValue}
        </div>
      )}

      {/* Ligne 3: Label (exercice/séance) - permet retour à la ligne */}
      <div className="text-theme-secondary text-xs break-words leading-tight">
        {data.label}
      </div>
      ...
      {data.secondaryLabel && (
        <div className="text-xs mt-0.5 whitespace-nowrap">
          <span className="text-theme-muted">{data.secondaryLabel}</span>{' '}
          <span className={`font-medium ${data.secondaryColor || 'text-theme'}`}>
            {data.secondaryValue}
          </span>
        </div>
      )}
  ...
  const baseClasses = "bg-theme-secondary border border-theme rounded-lg px-3 py-2 shadow-xl";
...
export function getRpeColor(rpe: number | null): string {
  if (rpe === null) return 'text-[var(--color-primary)]';
  if (rpe <= 4) return 'text-[var(--color-success)]';
