...
interface Props {
  ...
  /** Layout : 'mobile' (liste) ou 'desktop' (tuiles) - défaut: mobile */
...
export const PersonalRecordsList: React.FC<Props> = ({ userId, onViewSession, layout = 'mobile' }) => {
  ...
  // Filtre "récent" actif par défaut (mobile et desktop)
  ...
  // En mode desktop, on affiche toujours les résultats (filtre récent par défaut)
  const hasActiveFilters = layout === 'desktop' || searchQuery.length > 0 || activeFilter !== null;

  // Filtrer et trier
  const filteredRecords = useMemo(() => {
    // Si aucun filtre actif → liste vide (sauf en mode desktop)
    if (!hasActiveFilters) {
      return [];
    }
    ...
    // En mode desktop sans filtre explicite, trier par date décroissante par défaut
    if (layout === 'desktop' && !activeFilter && !searchQuery) {
      return results
        .filter(r => r.lastDate)
    ...
    return results;
  }, [records, searchQuery, activeFilter, hasActiveFilters, layout]);
  ...
  if (records.length === 0) {
    return (
      <div className="bg-theme-secondary/50 border border-theme rounded-xl p-8 text-center">
        <Trophy className="w-12 h-12 text-theme-muted mx-auto mb-3" />
        <p className="text-theme-muted">Aucun record pour l'instant</p>
        <p className="text-theme-muted text-sm mt-1">Complete des seances pour voir tes PRs</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-3 bg-theme-secondary/50 border border-theme rounded-xl text-theme placeholder-theme-muted focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50"
        />
        {searchQuery && (
          <button
            onClick={() => setSearchQuery('')}
            className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-theme-muted hover:text-theme-dark transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        )}
          ...
          <button
            ...
            className={`
              ...
              ${activeFilter === filter.id
                ? 'bg-purple-500 text-theme shadow-lg shadow-purple-500/20'
                : 'bg-theme-secondary/50 border border-theme text-theme-muted hover:bg-theme-secondary hover:text-theme-dark'
              }
            `}
          >
            {filter.icon}
      ...
      {hasActiveFilters && (
        <button
          ...
          className="text-sm text-theme-muted hover:text-theme-dark transition-colors flex items-center gap-1"
        >
          <X className="w-3 h-3" />
          Effacer les filtres
        </button>
      ...
      {!hasActiveFilters && (
        <div className="bg-theme-secondary/30 border border-theme rounded-xl p-8 text-center">
          <Search className="w-12 h-12 text-theme-muted mx-auto mb-3" />
          <p className="text-theme-muted">Utilise la recherche ou les filtres</p>
          <p className="text-theme-muted text-sm mt-1">pour voir tes records personnels</p>
        </div>
      )}
      ...
      {hasActiveFilters && filteredRecords.length === 0 && (
        <div className="bg-theme-secondary/30 border border-theme rounded-xl p-8 text-center">
          <Trophy className="w-12 h-12 text-theme-muted mx-auto mb-3" />
          <p className="text-theme-muted">Aucun exercice trouvé</p>
          <p className="text-theme-muted text-sm mt-1">Essaie avec d'autres critères</p>
        </div>
      )}
      ...
      {filteredRecords.length > 0 && (
        <>
          {layout === 'desktop' ? (
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
              {filteredRecords.map((record) => {
                ...
                const hasWeightData = record.maxWeight > 0 || record.maxVolume > 0 || record.max1RM > 0;
                const isTimeExercise = record.maxDuration > 0 && record.maxReps === 0;

                return (
                  <div
                    key={record.exerciseName}
                    className="bg-theme-secondary/50 border border-theme rounded-xl p-3 hover:bg-theme-secondary/70 transition-colors flex flex-col"
                  >
                    {/* Header: nom | date | œil */}
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h3 className="font-medium text-theme text-sm leading-tight line-clamp-2 flex-1">
                        {record.exerciseName}
                      </h3>
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        ...
                        {record.sessionLogId && onViewSession && (
                          <button
                            ...
                            className="p-1 bg-theme-tertiary/50 hover:bg-theme-tertiary rounded text-theme-muted hover:text-theme-dark transition-colors"
                    ...
                    {/* Muscle group */}
                    {record.muscleGroup && (
                      <span className="text-[10px] text-theme-muted mb-2 block">{record.muscleGroup}</span>
                    )}

                    {/* Encarts records */}
                    <div className="mt-auto">
                      {hasWeightData ? (
                        /* Poids | Volume | 1RM côte à côte */
                        <div className="flex gap-1">
                          {/* Poids */}
                          <div className="flex-1 bg-amber-500/10 border border-amber-500/20 rounded-lg p-1.5 text-center">
                            <Dumbbell className="w-3 h-3 text-amber-400 mx-auto mb-0.5" />
                            <span className="text-theme font-bold text-[11px] block">
                              {record.maxWeight > 0 ? formatWeight(record.maxWeight) : '-'}
                            </span>
                          </div>
                          {/* Volume */}
                          <div className="flex-1 bg-blue-500/10 border border-blue-500/20 rounded-lg p-1.5 text-center">
                          ...
                          {/* 1RM */}
                          <div className="flex-1 bg-orange-500/10 border border-orange-500/20 rounded-lg p-1.5 text-center">
                        ...
                        <div className="bg-purple-500/10 border border-purple-500/20 rounded-lg p-2 text-center">
                          <div className="flex items-center justify-center gap-1 text-purple-400 mb-0.5">
                            ...
                            <span className="text-[10px] font-medium">Durée max</span>
                          </div>
                          <span className="text-theme font-bold text-sm">{formatDuration(record.maxDuration)}</span>
                        </div>
                      ) : (
                        /* Reps max */
                        <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-2 text-center">
                          <div className="flex items-center justify-center gap-1 text-green-400 mb-0.5">
                            ...
                            <span className="text-[10px] font-medium">Reps max</span>
                          </div>
            ...
            <div className="space-y-3">
              {filteredRecords.map((record) => {
                ...
                return (
                  <div
                    key={record.exerciseName}
                    className="bg-theme-secondary/50 border border-theme rounded-xl p-4 hover:bg-theme-secondary/70 transition-colors"
                  >
                    {/* Exercise name with date and eye button */}
                    <div className="flex items-start justify-between mb-3 gap-2">
                      <div className="flex-1">
                        <h3 className="font-medium text-theme leading-tight">{record.exerciseName}</h3>
                        {record.muscleGroup && (
                          <span className="text-xs text-theme-muted">{record.muscleGroup}</span>
                        )}
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        {record.sessionLogId && onViewSession && (
                          <button
                            onClick={() => handleViewClick(record)}
                            className="p-1.5 bg-theme-tertiary/50 hover:bg-theme-tertiary rounded-lg text-theme-muted hover:text-theme-dark transition-colors"
          ...
          {/* Summary */}
          <div className="bg-theme-secondary/30 border border-theme rounded-xl p-4 text-center">
            <p className="text-theme-muted text-sm">
              <span className="text-theme font-medium">{filteredRecords.length}</span> exercice{filteredRecords.length > 1 ? 's' : ''} trouvé{filteredRecords.length > 1 ? 's' : ''}
            </p>
          </div>
        </>
      )}
          ...
          <div className="bg-theme-secondary border border-theme rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-theme">Voir la seance</h3>
                </div>
              </div>
              <button
                onClick={() => setShowViewModal(false)}
                className="p-2 text-theme-muted hover:text-theme-dark transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="bg-theme-secondary/50 rounded-xl p-3 mb-6">
              <p className="text-sm text-theme-muted">Exercice</p>
              <p className="text-theme font-medium">{selectedRecord.exerciseName}</p>
              <p className="text-xs text-theme-muted mt-1">{formatDate(selectedRecord.lastDate)}</p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3 bg-purple-600 hover:bg-purple-500 text-theme rounded-xl font-medium transition-colors"
              >
                Voir
              </button>
            </div>
