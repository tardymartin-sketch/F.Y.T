...
function getRpeColor(rpe: number | null, hasTraining: boolean): string {
  if (!hasTraining) return 'bg-theme-secondary';
...
function getRpeTextColor(rpe: number | null, hasTraining: boolean): string {
  if (!hasTraining) return 'text-theme-muted';
...
export const HeatmapCalendar: React.FC<Props> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        className="bg-theme-secondary/50 border border-theme rounded-xl p-4 relative"
        ref={containerRef}
        onClick={() => {
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 bg-theme-tertiary hover:opacity-90 rounded-lg transition-colors"
            >
              <ChevronLeft className="w-4 h-4 text-theme-secondary" />
            </button>
            <span className="text-theme font-medium w-16 text-center">{year}</span>
            <button
              onClick={() => setYear(year + 1)}
              disabled={year >= new Date().getFullYear()}
              className="p-2 bg-theme-tertiary hover:opacity-90 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        {/* Monthly blocks grid - 2 columns on mobile, 3 on tablet, 6 on desktop (6x2) */}
        <div className={`grid gap-3 ${
          isDesktop
            ? 'grid-cols-6' // Desktop: grille 6x2 (6 mois par ligne)
            : 'grid-cols-2 sm:grid-cols-3' // Mobile/tablet: comportement original
        }`}>
          {monthsData.map((monthData) => (
            <div key={monthData.month} className="bg-theme/50 rounded-lg p-2">
              {/* Month name */}
              <div className="text-xs font-medium text-theme-secondary text-center mb-2">
                {monthData.name}
              </div>
              ...
              <div className="grid grid-cols-7 gap-0.5 mb-1">
                {DAYS_FR.map((day, i) => (
                  <div
                    key={i}
                    className="text-[8px] text-theme-muted text-center"
                  >
                    {day}
                  </div>
                ))}
        ...
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-theme">
          <div className="flex items-center gap-1 text-[10px]">
            <div className="w-3 h-3 rounded-sm bg-theme-secondary" />
            <span className="text-theme-muted">Repos</span>
          </div>
          <div className="flex items-center gap-1 text-[10px]">
            <div className="w-3 h-3 rounded-sm bg-blue-600" />
            <span className="text-theme-muted">Sans RPE</span>
          </div>
          ...
          <span className="text-theme-muted text-[10px]">RPE ↑</span>
        </div>
      </div>

      {/* Stats summary */}
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-theme-secondary/50 border border-theme rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-[var(--color-success)]">{totalSessions}</div>
          <div className="text-theme-muted text-sm">Séances</div>
        </div>
        <div className="bg-theme-secondary/50 border border-theme rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-[var(--color-primary)]">{formatVolume(totalVolume)}</div>
          <div className="text-theme-muted text-sm">Volume total</div>
        </div>
      </div>
    </div>
  );
