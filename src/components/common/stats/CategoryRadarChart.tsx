...
export const CategoryRadarChart: React.FC<Props> = ({ userId }) => {
  ...
  if (data.length === 0) {
    return (
      <div className="bg-theme-secondary/50 border border-theme rounded-xl p-8 text-center">
        <Target className="w-12 h-12 text-theme-muted mx-auto mb-3" />
        <p className="text-theme-muted">Aucune donnée de distribution</p>
        <p className="text-theme-muted text-sm mt-1">Complète des séances pour voir ton ratio</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Category breakdown */}
      <div className="bg-theme-secondary/50 border border-theme rounded-xl p-4">
        <h3 className="text-theme font-medium mb-3">Détail par catégorie</h3>
        ...
        <div className="space-y-3">
          {rawData.map((cat) => (
            <div key={cat.category} className="flex items-center gap-3">
              ...
              />
              <span className="text-theme text-sm flex-1 truncate">
                {CATEGORY_LABELS[cat.category] || cat.category}
              </span>
              <span className="text-theme-muted text-sm">{formatVolume(cat.totalVolume)}</span>
              <span className="text-theme-muted text-xs w-12 text-right">
                {cat.percentage}%
              </span>
            </div>
          ))}
        ...
        <div className="mt-4 pt-3 border-t border-theme flex items-center justify-between">
          <span className="text-theme-muted text-sm">Volume total</span>
          <span className="text-theme font-medium">{formatVolume(totalVolume)}</span>
        </div>
      </div>
    </div>
  );
