...
export const SessionPreviewModal: React.FC<Props> = ({
  ...
  const modalContent = (
    <>
      {/* Header - sans X en mode Popover (géré par Popover) */}
      <div className="sticky top-0 z-10 bg-theme-secondary border-b border-theme/50 p-4">
        <div className="flex items-center justify-between">
          <div className="flex-1 min-w-0 pr-4">
            <h2
              id="session-preview-title"
              className="text-xl font-bold text-theme truncate"
            >
              {sessionName}
            </h2>
          ...
          {!anchorRef && (
            <button
              ...
              onClick={onClose}
              className="
                p-2 rounded-xl bg-theme-tertiary text-theme-muted
                hover:bg-theme-secondary hover:text-theme
      ...
      {/* Exercise List */}
      <div className="overflow-y-auto p-4 space-y-3" style={{ maxHeight: anchorRef ? '50vh' : 'calc(85dvh - 160px)' }}>
      ...
      {/* Footer */}
      <div className="sticky bottom-0 z-10 bg-theme-secondary border-t border-theme/50 p-4">
        <button
          onClick={handleStart}
          className="
            w-full flex items-center justify-center gap-3
            py-3 rounded-xl font-semibold text-theme
            bg-gradient-to-r from-blue-600 to-emerald-600
            hover:from-blue-500 hover:to-emerald-500
            shadow-lg shadow-blue-600/25
            transition-all duration-200
            active:scale-[0.98]
            focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:ring-offset-2 focus:ring-offset-theme-secondary
          "
        >
  ...
  return (
    ...
    >
      ...
      <div
        ref={modalRef}
        className="
          relative w-full max-w-lg bg-theme-secondary border border-theme/50
...
const ExerciseCard: React.FC<ExerciseCardProps> = ({ exercise, index }) => {
  ...
  return (
    <div className="bg-theme-tertiary/50 rounded-xl p-4 border border-theme/50 transition-all hover:border-theme">
      {/* Header */}
      <div className="flex items-start gap-3">
        {/* Index */}
        <div className="
          w-8 h-8 rounded-lg bg-theme-tertiary flex items-center justify-center
          text-sm font-semibold text-theme flex-shrink-0
        ">
          {index}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Metrics */}
          <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-theme-muted">
            {/* Sets x Reps */}
