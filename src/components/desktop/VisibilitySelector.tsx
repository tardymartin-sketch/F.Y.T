    ...
    <div className="space-y-4">
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all ${
              visibilityType === 'all'
                ...
                : 'border-theme hover:border-theme bg-theme-secondary/50'
            }`}
          >
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
              visibilityType === 'all' ? 'bg-blue-500/20' : 'bg-theme-tertiary'
            }`}>
              <Globe className={`w-5 h-5 ${visibilityType === 'all' ? 'text-blue-400' : 'text-theme-muted'}`} />
            </div>
            <div className="text-left flex-1">
              <p className={`font-medium ${visibilityType === 'all' ? 'text-theme' : 'text-theme'}`}>
                Tous mes athlètes
              </p>
              <p className="text-xs text-theme-muted">Visible par tous</p>
            </div>
          ...
          >
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
              visibilityType === 'groups' ? 'bg-emerald-500/20' : 'bg-theme-tertiary'
            }`}>
              <Users className={`w-5 h-5 ${visibilityType === 'groups' ? 'text-emerald-400' : 'text-theme-muted'}`} />
            </div>
            <div className="text-left flex-1">
              <p className={`font-medium ${visibilityType === 'groups' ? 'text-theme' : 'text-theme'}`}>
                Groupes spécifiques
              </p>
          ...
          >
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
              visibilityType === 'athletes' ? 'bg-purple-500/20' : 'bg-theme-tertiary'
            }`}>
              <User className={`w-5 h-5 ${visibilityType === 'athletes' ? 'text-purple-400' : 'text-theme-muted'}`} />
            </div>
            <div className="text-left flex-1">
              <p className={`font-medium ${visibilityType === 'athletes' ? 'text-theme' : 'text-theme'}`}>
      ...
      {visibilityType === 'groups' && (
        <div>
          <label className="block text-sm font-medium text-theme-muted mb-2">Sélectionner les groupes</label>
          {availableGroups.length === 0 ? (
            <div className="bg-theme-secondary/50 border border-theme rounded-lg p-4 text-center text-theme-muted text-sm">
              Aucun groupe disponible. Créez des groupes dans l'onglet Équipe.
            </div>
          ) : (
            <div className="bg-theme-secondary border border-theme rounded-lg divide-y divide-theme">
              {availableGroups.map((group) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-theme-tertiary transition-colors ${
                      isSelected ? 'bg-theme-tertiary' : ''
                    }`}
                  >
                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${
                      isSelected ? 'bg-emerald-500 border-emerald-500' : 'border-theme'
                    }`}>
                    ...
                    <span className="text-theme text-sm">{group.name}</span>
                    <span className="text-xs text-theme-muted ml-auto">{group.memberCount} membres</span>
                  </button>
                );
              })}
            </div>
      ...
      {visibilityType === 'athletes' && (
        <div>
          <label className="block text-sm font-medium text-theme-muted mb-2">Sélectionner les athlètes</label>
          {availableAthletes.length === 0 ? (
            <div className="bg-theme-secondary/50 border border-theme rounded-lg p-4 text-center text-theme-muted text-sm">
              Aucun athlète disponible.
            </div>
          ) : (
            <div className="bg-theme-secondary border border-theme rounded-lg divide-y divide-theme max-h-64 overflow-y-auto">
              {availableAthletes.map((athlete) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${
                      isSelected ? 'bg-purple-500 border-purple-500' : 'border-theme'
                    }`}>
                      {isSelected && <Check className="w-3 h-3 text-theme" />}
                    </div>
                    <div className="w-8 h-8 bg-theme-tertiary rounded-lg flex items-center justify-center text-theme-muted text-xs font-medium">
