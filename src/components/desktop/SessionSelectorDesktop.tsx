  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-3 gap-2 md:gap-4">
        {/* Année */}
        <div>
          <label className="block text-xs md:text-sm font-medium text-theme-muted mb-1 md:mb-2">Année</label>
          <div className="relative">
            <select 
              className="w-full bg-theme-tertiary/80 border border-theme text-theme rounded-lg md:rounded-xl px-2 md:px-4 py-2 md:py-3 pr-6 md:pr-10 text-sm md:text-base appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all cursor-pointer"
            ...
            <ChevronDown className="absolute right-1.5 md:right-3 top-1/2 -translate-y-1/2 w-4 h-4 md:w-5 md:h-5 text-theme-muted pointer-events-none" />
          </div>
        </div>

        {/* Mois */}
        <div>
          <label className="block text-xs md:text-sm font-medium text-theme-muted mb-1 md:mb-2">Mois</label>
          <div className="relative">
            <select 
              className="w-full bg-theme-tertiary/80 border border-theme text-theme rounded-lg md:rounded-xl px-2 md:px-4 py-2 md:py-3 pr-6 md:pr-10 text-sm md:text-base appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        {/* Semaine */}
        <div>
          <label className="block text-xs md:text-sm font-medium text-theme-muted mb-1 md:mb-2">Semaine</label>
          <div className="relative">
            <select 
              className="w-full bg-theme-tertiary/80 border border-theme text-theme rounded-lg md:rounded-xl px-2 md:px-4 py-2 md:py-3 pr-6 md:pr-10 text-sm md:text-base appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <div>
        <div className="flex items-center justify-between mb-2 md:mb-3">
          <label className="text-xs md:text-sm font-medium text-theme-muted">
            Sessions
          </label>
          {sessions.length > 0 && (
              ...
              {filters.selectedSeances.length > 0 && (
                <>
                  ...
                  <button 
                    ...
                    className="text-xs text-theme-muted hover:text-theme transition-colors"
                  >
                    Effacer
                  </button>
                </>
        ...
        {!filters.selectedSemaine ? (
          <div className="bg-theme-tertiary/30 border border-dashed border-theme rounded-lg md:rounded-xl p-4 md:p-6 text-center">
            <Calendar className="w-6 h-6 md:w-8 md:h-8 text-theme-muted mx-auto mb-1 md:mb-2" />
            <p className="text-theme-muted text-xs md:text-sm">Sélectionnez une semaine</p>
          </div>
        ) : sessions.length === 0 ? (
          <div className="bg-theme-tertiary/30 border border-dashed border-theme rounded-lg md:rounded-xl p-4 md:p-6 text-center">
            <p className="text-theme-muted text-xs md:text-sm">Aucune session trouvée</p>
          </div>
        ) : (
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${isSelected 
                      ? 'bg-blue-600 border-blue-500 text-theme shadow-lg shadow-blue-600/25' 
                      : 'bg-theme-secondary/50 border-theme text-theme hover:border-blue-500/50 hover:text-theme'
                    }
                  `}
                >
                  <span>Session {s}</span>
                  {isSelected && (
                    <span className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-emerald-500 rounded-full flex items-center justify-center text-[10px] font-bold text-theme shadow-lg">
                      {selectionIndex + 1}
                    </span>
                  )}
                </button>
