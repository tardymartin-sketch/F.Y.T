...
export const TeamView: React.FC<Props> = ({
  ...
  const loadComments = async () => {
    ...
    console.log(
      "[TeamView.loadComments] Chargement commentaires pour coach:",
      coachId,
      "showAllComments:",
      showAllComments,
    );
    ...
    try {
      ...
      console.log(
        "[TeamView.loadComments] Commentaires reçus:",
        data.length,
        data,
      );
      setComments(data);
      ...
      console.error(
        "[TeamView.loadComments] Erreur chargement commentaires:",
        e,
      );
    } finally {
  ...
  const handleSaveOrganizer = async () => {
    ...
    if (
      !organizerForm.title ||
      !organizerForm.startDate ||
      !organizerForm.endDate ||
      !organizerForm.message
    ) {
    ...
    try {
      const newLog: WeekOrganizerLog = {
        ...
        visibleToGroupIds:
          organizerForm.visibilityType === "groups"
            ? organizerForm.visibleToGroupIds
            : undefined,
        visibleToAthleteIds:
          organizerForm.visibilityType === "athletes"
            ? organizerForm.visibleToAthleteIds
            : undefined,
      };
  ...
  const getAthleteStats = (history: SessionLog[]) => {
    ...
    const sessionsWithRpe = recentSessions.filter(
      (h) => h.sessionRpe !== undefined,
    );
    const avgRpe =
      sessionsWithRpe.length > 0
        ? Math.round(
            (sessionsWithRpe.reduce((acc, h) => acc + (h.sessionRpe || 0), 0) /
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <LexicalRichTextEditor
                  ...
                  onChange={(html) =>
                    setOrganizerForm((prev) => ({ ...prev, message: html }))
                  }
      ...
      <div className="space-y-4">
        {organizerLogs.length === 0 ? (
          ...
          organizerLogs.map((log) => {
            const isActive =
              new Date(log.startDate) <= new Date() &&
              new Date(log.endDate) >= new Date();
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-theme-muted mb-2">
                      ...
                      {new Date(log.endDate).toLocaleDateString("fr-FR")}
                    </p>
            ...
            >
              {filteredAthletes.map((athlete) => (
                ...
                >
                  ...
                  >
                    {athlete.firstName?.[0]}
                    {athlete.lastName?.[0] || athlete.username[0]}
                  </div>
          ...
          {!selectedAthlete ? (
            <div className="bg-theme-tertiary border border-dashed border-theme rounded-2xl p-12 text-center">
              ...
              <p className="text-theme-muted">
                Sélectionnez un athlète pour voir son historique
              </p>
            </div>
          ) : loading ? (
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="bg-theme-tertiary rounded-xl p-4 text-center">
                      {stats.avgRpe !== null ? (
                        <>
                          <p
                            className={`text-2xl font-bold ${getRpeColor(stats.avgRpe)}`}
                          >
                            {stats.avgRpe}
                          </p>
                        ...
                        >
                          ...
                          >
                            <div className="flex items-center gap-4">
                              <div className="text-center min-w-[50px]">
                                <span className="block text-2xl font-bold text-theme">
                                  {date.getDate()}
                                </span>
                          ...
                          {isExpanded && (
                            <div className="px-4 pb-4 space-y-3 border-t border-theme">
                              ...
                              {session.sessionRpe && (
                                <div
                                  className={`mt-4 p-3 rounded-xl ${getRpeBgColor(session.sessionRpe)} flex items-center justify-between`}
                                >
                                  <div className="flex items-center gap-2">
                                    <Gauge
                                      className={`w-5 h-5 ${getRpeColor(session.sessionRpe)}`}
                                    />
                              ...
                              {session.exercises.map((ex, exIdx) => (
                                <div
                                  key={exIdx}
                                  className="bg-theme-tertiary rounded-lg p-3 mt-3"
                                >
                                  ...
                                  {ex.notes && (
                                    <p className="mt-2 text-xs text-theme-muted italic">
                                      {ex.notes}
                                    </p>
                                  )}
                                </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-theme-muted mt-1">
              Gérez vos athlètes et leurs entraînements
            </p>
          </div>
        </div>
      </div>
