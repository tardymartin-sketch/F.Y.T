...
export const StravaHistoryCard: React.FC<StravaHistoryCardProps> = ({ 
  ...
  if (!log || !log.exercises || log.exercises.length === 0) {
    return (
      <div className="bg-theme-secondary border border-theme rounded-2xl p-4 text-center text-theme-muted">
  ...
  return (
    <div 
      className={`bg-theme-secondary border rounded-2xl overflow-hidden transition-all duration-200 ${
        isExpanded ? 'border-orange-500/50' : 'border-theme hover:border-theme'
      }`}
    >
      {/* Header */}
      <button
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="w-16 text-center">
            <div className="bg-theme-tertiary rounded-xl p-3">
              <span className="text-xs text-theme-muted uppercase block">{dateInfo.month}</span>
              <span className="text-2xl font-bold text-theme block">{dateInfo.day}</span>
            </div>
          </div>

          <div>
            <h3 className="font-semibold text-theme text-lg flex items-center gap-2">
      ...
      {isExpanded && (
        <div className="px-5 pb-5 space-y-4 border-t border-theme pt-4">
          ...
          <div className="sm:hidden flex flex-wrap gap-2">
            ...
            {avgHr && (
              <div className="flex items-center gap-1.5 text-red-400 bg-theme-tertiary px-3 py-1.5 rounded-lg text-sm">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-colors ${
                !showCharts ? 'bg-orange-500 text-theme' : 'bg-theme-tertiary text-theme hover:bg-theme-secondary'
              }`}
            >
              <MapPin className="w-4 h-4" />
              Carte
          ...
          {isLoading && (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-2 text-theme-muted">Chargement des données...</span>
            </div>
          )}
          ...
          {!showCharts && !isLoading && (
            stravaData?.summary_polyline ? (
              ...
              <div className="h-32 bg-theme-tertiary/50 rounded-xl flex items-center justify-center text-theme-muted text-sm">
          ...
          {log.comments && (
            <div className="bg-theme-tertiary/30 rounded-xl p-4">
              <p className="text-theme-muted text-sm whitespace-pre-wrap">{log.comments}</p>
            </div>
          )}
        </div>
      )}
