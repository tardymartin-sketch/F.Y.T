...
import { ConversationThread, ThreadMessage } from '../common/ConversationThread';
...
interface Props {
  ...
  /**
   * Mode d'affichage:
   * - 'hierarchical': Athlète → Exercice → Messages (pour coach)
   * - 'flat': Exercice → Messages directement (pour athlete/admin)
   */
...
/**
 * Mode flat : retourne une liste plate de SessionGroup (exercices) triée par date
 */
function getFlatExerciseList(
  comments: AthleteComment[],
  currentUserId: string
): SessionGroup[] {
  // Grouper par exerciseName
  const exerciseMap = new Map<string, AthleteComment[]>();
  comments.forEach(comment => {
    const key = comment.exerciseName;
    if (!exerciseMap.has(key)) {
...
// ===========================================
// EXERCISE ITEM COMPONENT (Mode flat)
// ===========================================
...
const ExerciseItem: React.FC<ExerciseItemProps> = ({
  ...
  return (
    <button
      onClick={onSelect}
      className={`
        w-full flex items-center gap-3 p-4 rounded-xl transition-all duration-200 mb-2
        ${isSelected
    ...
    >
      ...
      <div className={`
        ...
        ${session.unreadCount > 0 ? 'bg-emerald-500/20' : 'bg-theme-tertiary/50'}
      `}>
        <Dumbbell className={`w-6 h-6 ${session.unreadCount > 0 ? 'text-[var(--color-success)]' : 'text-theme-muted'}`} />
      </div>

      {/* Info */}
      <div className="flex-1 min-w-0 text-left">
        <div className="flex items-center justify-between">
          <h3 className={`font-medium truncate ${session.unreadCount > 0 ? 'text-theme' : 'text-theme'}`}>
            {session.exerciseName}
          </h3>
          <span className={`text-xs ml-2 ${session.unreadCount > 0 ? 'text-[var(--color-primary)]' : 'text-theme-muted'}`}>
        ...
        {session.sessionName && (
          <div className="flex items-center gap-1 mt-0.5">
            ...
            <span className="text-xs text-theme-muted truncate">{session.sessionName}</span>
          </div>
        )}
      ...
      <div className="flex items-center gap-2">
        {session.unreadCount > 0 && (
          <span className="flex items-center justify-center min-w-[20px] h-5 px-1.5 bg-red-500 rounded-full text-xs font-bold text-theme">
...
// ===========================================
// ATHLETE ITEM COMPONENT (Mode hierarchical)
// ===========================================
...
const AthleteItem: React.FC<AthleteItemProps> = ({
  ...
  return (
    <div className="border-b border-theme last:border-b-0">
      ...
      <button
        onClick={onToggle}
        className="w-full flex items-center gap-3 p-4 hover:bg-theme-tertiary/50 transition-colors"
      >
        {/* Avatar */}
        <div className={`
          w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0
          ${athlete.unreadCount > 0 ? 'bg-blue-500/20' : 'bg-theme-tertiary/50'}
        `}>
          <User className={`w-6 h-6 ${athlete.unreadCount > 0 ? 'text-[var(--color-primary)]' : 'text-theme-muted'}`} />
        </div>

        {/* Info */}
        <div className="flex-1 min-w-0 text-left">
          <div className="flex items-center justify-between">
            <h3 className={`font-medium truncate ${athlete.unreadCount > 0 ? 'text-theme' : 'text-theme'}`}>
              {displayName}
            </h3>
            <span className={`text-xs ml-2 ${athlete.unreadCount > 0 ? 'text-[var(--color-primary)]' : 'text-theme-muted'}`}>
      ...
      {isExpanded && (
        <div className="bg-theme-tertiary/30 border-t border-theme p-2">
          {athlete.sessions.map((session, index) => {
            const isSelected = session.exerciseName === selectedExerciseName;
            return (
            <button
              key={`${session.exerciseName}-${index}`}
              onClick={() => onSelectSession(session)}
              className={`w-full flex items-center gap-3 px-4 py-3 pl-6 rounded-xl transition-all duration-200 mb-2 last:mb-0 ${
                isSelected
                  ? 'bg-blue-600/20 border-2 border-blue-500'
                  : 'bg-theme-secondary/50 border border-theme hover:bg-theme-secondary hover:border-theme'
              }`}
            >
              ...
              <div className={`
                ...
                ${session.unreadCount > 0 ? 'bg-emerald-500/20' : 'bg-theme-tertiary/50'}
              `}>
                <Dumbbell className={`w-5 h-5 ${session.unreadCount > 0 ? 'text-[var(--color-success)]' : 'text-theme-muted'}`} />
              </div>

              {/* Session Info */}
              <div className="flex-1 min-w-0 text-left">
                <div className="flex items-center gap-2">
                  <h4 className={`font-medium truncate ${session.unreadCount > 0 ? 'text-theme' : 'text-theme'}`}>
                    {session.exerciseName}
                  </h4>
                </div>
                {session.sessionName && (
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                {session.unreadCount > 0 && (
                  <span className="flex items-center justify-center min-w-[18px] h-[18px] px-1 bg-red-500 rounded-full text-[10px] font-bold text-theme">
                    {session.unreadCount}
                  </span>
                )}
              </div>
...
export const CoachConversationsView: React.FC<Props> = ({
  ...
  // V3: État expanded des athlètes avec persistance via store singleton (mode hierarchical)
  ...
  // Mode hierarchical: Grouper par athlète puis exercice
  const athleteGroups = useMemo(
    () => groupCommentsByAthleteAndSession(comments, currentUserId),
    [comments, currentUserId]
  );

  // Mode flat: Liste plate d'exercices
  const flatExercises = useMemo(
    () => getFlatExerciseList(comments, currentUserId),
    [comments, currentUserId]
  );

  // Toggle athlete expansion (mode hierarchical seulement)
  ...
  // Open thread (mode hierarchical)
  const handleSelectSession = (athlete: AthleteGroup, session: SessionGroup) => {
    setActiveThread({ athlete, session });
  };

  // Open thread (mode flat) - utilise un athlete fictif car non affiché
  const handleSelectExercise = (session: SessionGroup) => {
    const dummyAthlete: AthleteGroup = {
    ...
    setActiveThread({ athlete: dummyAthlete, session });
  };
  ...
  // Calculate totals selon le mode
  const totalUnread = displayMode === 'hierarchical'
    ? athleteGroups.reduce((acc, a) => acc + a.unreadCount, 0)
    : flatExercises.reduce((acc, s) => acc + s.unreadCount, 0);

  // Get athlete name for active thread (mode hierarchical only)
  const activeAthleteName = activeThread && displayMode === 'hierarchical' ? getAthleteName({
  ...
  // Titre du thread selon le mode
  const threadTitle = activeThread
    ? displayMode === 'hierarchical'
      ? `${activeAthleteName} - ${activeThread.session.exerciseName}`
      : activeThread.session.exerciseName
    : '';

  // ===========================================
  // RENDER - Layout split-screen desktop
  // ===========================================
        ...
        >
          <div className="flex items-center justify-between gap-4 pl-12">
            ...
            <div className="flex items-center gap-3">
              <MessageSquare className="w-6 h-6 text-[var(--color-primary)] flex-shrink-0" />
              <div>
                <h1 className="text-xl font-bold text-theme">Messages</h1>
                <p className="text-sm text-theme-muted">
                  {displayMode === 'hierarchical'
                    ? `${athleteGroups.length} athlète${athleteGroups.length > 1 ? 's' : ''}`
                    : `${flatExercises.length} conversation${flatExercises.length > 1 ? 's' : ''}`
                  }
            ...
            {/* Badge non lus */}
            {totalUnread > 0 && (
              <span className="flex items-center justify-center min-w-[28px] h-7 px-2 bg-red-500 rounded-full text-sm font-bold text-white">
        ...
        >
          ...
          >
            ...
            >
              ...
              ) : (displayMode === 'hierarchical' ? athleteGroups.length : flatExercises.length) === 0 ? (
                <div className="p-8 text-center">
                  <MessageSquare className="w-12 h-12 text-theme-muted mx-auto mb-3" />
                  <p className="text-theme-muted">Aucune conversation</p>
                ...
                <div>
                  {athleteGroups.map(athlete => (
                    <AthleteItem
                      ...
                      selectedExerciseName={activeThread?.session.exerciseName}
                    />
                  ))}
                </div>
              ) : (
                <div>
                  {flatExercises.map((session, index) => (
                    <ExerciseItem
                      key={`${session.exerciseName}-${index}`}
                      session={session}
                      isSelected={activeThread?.session.exerciseName === session.exerciseName}
                      onSelect={() => handleSelectExercise(session)}
                    />
                  ))}
                </div>
              )}
          ...
          {/* ========== PANNEAU DROIT : CONVERSATION THREAD ========== */}
          <div
            className="bg-theme-secondary/50 border border-theme rounded-xl"
            style={{
              width: '60%',
            ...
            {activeThread ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-theme-muted">
                ...
                <p className="text-sm mt-1">
                  {displayMode === 'hierarchical'
                    ? "Cliquez sur un athlète puis sur une conversation pour afficher les messages"
