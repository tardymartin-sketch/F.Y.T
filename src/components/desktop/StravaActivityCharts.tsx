...
export const StravaActivityCharts: React.FC<StravaActivityChartsProps> = ({
  ...
  if (chartData.length === 0) {
    return (
      <div className="h-32 bg-theme-secondary/50 rounded-xl flex items-center justify-center text-theme-muted text-sm">
      ...
      {hasHeartrate && (
        <div className="bg-theme-tertiary/30 rounded-xl p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-theme">Fréquence cardiaque</span>
          </div>
      ...
      {hasAltitude && (
        <div className="bg-theme-tertiary/30 rounded-xl p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-theme">Altitude</span>
          </div>
          <ResponsiveContainer width="100%" height={chartHeight}>
            <AreaChart data={chartData}>
              <defs>
      ...
      {hasSpeed && (
        <div className="bg-theme-tertiary/30 rounded-xl p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-theme">Vitesse</span>
          </div>
          <ResponsiveContainer width="100%" height={chartHeight}>
      ...
      {hasPower && (
        <div className="bg-theme-tertiary/30 rounded-xl p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium text-theme">Puissance</span>
          </div>
          <ResponsiveContainer width="100%" height={chartHeight}>
            <LineChart data={chartData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
      ...
      {segments && segments.length > 0 && !compact && (
        <div className="bg-theme-tertiary/30 rounded-xl p-3">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-theme">Segments ({segments.length})</span>
          </div>
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {segments.map((seg) => (
              <div key={seg.id} className="bg-theme-secondary/50 rounded-lg p-2 text-sm">
                <div className="flex items-center justify-between">
                  <span className="text-theme font-medium truncate">{seg.segment.name}</span>
                  {seg.pr_rank && (
                    <span className={`px-2 py-0.5 rounded text-xs ${
                      seg.pr_rank === 1 ? 'bg-yellow-500/20 text-yellow-400' :
                      seg.pr_rank === 2 ? 'bg-theme-tertiary/20 text-theme' :
