...
export const SessionView: React.FC<Props> = ({ sessionData, selectedSessionsOrder }) => {
  if (sessionData.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-theme-muted bg-theme-secondary/30 rounded-xl border border-dashed border-theme">
  ...
  return (
    <div className="animate-fade-in">
      {/* En-tête */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 border-b border-theme pb-6 pl-12">
        <div>
          <div className="flex items-center gap-2 text-blue-500 mb-1 text-sm font-semibold tracking-wide uppercase">
            ...
            <span className="w-1 h-1 bg-theme-muted rounded-full"></span>
          ...
          <p className="text-theme-muted mt-1">{sortedData.length} exercices</p>
        </div>
      </div>
        ...
        {sortedData.map((exercise, idx) => {
          ...
          return (
            <div 
              ...
              className="group bg-theme-secondary border border-theme rounded-xl overflow-hidden hover:border-blue-500/30 transition-all duration-300 relative"
            >
              {/* Badge d'ordre */}
              <div className="absolute top-0 left-0 bg-theme-tertiary text-theme-muted px-3 py-1 text-xs font-mono border-b border-r border-theme rounded-br-lg group-hover:bg-blue-900/50 group-hover:text-blue-400 transition-colors flex gap-2">
              ...
              <div className="p-5 pt-8 md:p-6 md:pt-6 flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1">
                  <h3 className="text-xl font-bold text-theme mb-4">{exercise.exercice}</h3>
                  
                  {/* Métriques */}
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    <div className="bg-theme/50 p-3 rounded-lg border border-theme/50">
                      <span className="text-xs text-theme-muted block mb-1 uppercase tracking-wider">Séries</span>
                      <span className="font-mono text-lg text-blue-400 font-semibold">{exercise.series || '-'}</span>
                    </div>
                    <div className="bg-theme/50 p-3 rounded-lg border border-theme/50">
                      <span className="text-xs text-theme-muted block mb-1 uppercase tracking-wider">Reps/Durée</span>
                      <span className="font-mono text-lg text-emerald-400 font-semibold">{exercise.repsDuree || '-'}</span>
                    </div>
                    <div className="bg-theme/50 p-3 rounded-lg border border-theme/50">
                      <span className="text-xs text-theme-muted block mb-1 uppercase tracking-wider">Repos</span>
                      <div className="flex items-center gap-1.5">
                        <Clock className="w-4 h-4 text-theme-muted" />
                        <span className="font-mono text-lg text-theme">{exercise.repos || '-'}s</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-4 text-sm text-theme-muted">
                    {exercise.tempoRpe && (
                      <div className="flex items-center gap-2 bg-theme-secondary/50 px-3 py-1.5 rounded-full">
                        <Activity className="w-4 h-4 text-orange-400" />
                        <span className="text-theme">
                          Tempo/RPE: <span className="text-theme font-medium">{exercise.tempoRpe}</span>
                        </span>
                      </div>
                    )}
                    {hasValidVideo && (
                      <a 
                        ...
                        className="flex items-center gap-2 bg-theme-secondary/50 px-3 py-1.5 rounded-full hover:bg-blue-900/20 hover:text-blue-400 transition-colors"
                      >
                ...
                {(exercise.notes || exercise.retour) && (
                  <div className="md:w-1/3 border-l border-theme pl-6 border-dashed flex flex-col gap-4">
                    {exercise.notes && (
                      <div className="bg-yellow-900/10 border border-yellow-900/20 p-3 rounded-lg">
                        ...
                        <p className="text-sm text-theme leading-relaxed">{exercise.notes}</p>
                      </div>
                    )}
                    {exercise.retour && (
                      <div className="bg-purple-900/10 border border-purple-900/20 p-3 rounded-lg">
                        ...
                        <p className="text-sm text-theme italic">"{exercise.retour}"</p>
                      </div>
                    )}
                  </div>
                )}
