...
export const CoachTab: React.FC<Props> = ({
  ...
  if (!coachId) {
    return (
      <div className={`space-y-6 ${className}`}>
        <div className="bg-theme-secondary border border-theme rounded-2xl p-8 text-center">
  ...
  if (activeThread) {
    // Desktop adaptation - hauteur différente selon le layout
    const containerHeight = layout === 'desktop'
      ? 'h-[calc(100vh-120px)]'
      : 'h-[calc(100vh-180px)]'; // Mobile: prend en compte la BottomNav
