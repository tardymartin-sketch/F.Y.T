...
const stateStyles: Record<SessionState, {
  ...
  none: {
    ...
    shadow: 'shadow-lg shadow-[var(--shadow-color)]',
    icon: Play,
    pulse: false,
    label: 'Démarrer une séance',
  },
  active: {
    gradient: 'bg-gradient-to-r from-[var(--color-warning)] to-amber-500',
    shadow: 'shadow-lg shadow-[var(--color-warning)]/40',
    icon: Timer,
    pulse: true,
    ...
    >
      <button
        ...
        className={`
          ...
          focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:ring-offset-2 focus:ring-offset-theme
      ...
      >
        ...
        {style.pulse && (
          <>
            ...
            <span className="absolute -top-1 -right-1 w-3 h-3 bg-[var(--color-danger)] rounded-full border-2 border-theme" />
          </>
        )}
      </button>
      ...
      {sessionState !== 'none' && (
        <div className="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap">
          <span className="px-2 py-1 text-xs font-medium text-[var(--color-warning)] bg-theme-secondary rounded-full border border-[var(--color-warning)]/30">
            Session en cours
          </span>
        </div>
      )}
