...
interface Props {
  ...
  columns?: number; // Nombre de colonnes (default: 3 pour mobile)
}
...
export const SessionBadgesGrid: React.FC<Props> = ({
  ...
  weekStartDate,
  weekEndDate,
  columns = 3, // Desktop adaptation - default preserves mobile behavior
}) => {
    ...
    <div className={className}>
      <div className={`grid ${gridColsClass} gap-2`}>
            ...
            <button
              ...
              className={`
                ...
                ${isSelected
                  ...
                  : isCompleted
                    ? 'bg-theme-tertiary text-theme-muted line-through hover:bg-[var(--color-border-hover)]'
                    : 'bg-theme-tertiary text-theme-secondary hover:bg-[var(--color-border-hover)]'
                }
              `}
            >
              {/* Icône de sélection */}
              ...
              {isCompleted && (
                <CheckCircle2 className={`w-3.5 h-3.5 flex-shrink-0 ${isSelected ? 'text-white' : 'text-[var(--color-success)]'}`} />
              )}
              ...
              {isSelected && selectionOrder.has(session.name) && (
                <span className="absolute -top-2 -right-2 w-5 h-5 bg-white text-[var(--color-primary)] text-xs font-bold rounded-full flex items-center justify-center shadow-md ring-2 ring-[var(--color-primary)]">
              ...
              {!isSelected && isSuggested && (
                <span className="absolute -top-1 -right-1 w-2 h-2 bg-[var(--color-success)] rounded-full animate-pulse" />
              )}
            </button>
          );
        })}
        ...
        {!showAll && hasMore && (
          <button
            ...
            className="flex items-center justify-center px-2.5 py-2 rounded-xl text-xs font-medium bg-theme-tertiary text-theme-muted hover:bg-[var(--color-border-hover)] hover:text-theme transition-all"
          >
            ...
          </button>
        )}
      ...
      {showAll && hasMore && (
        <button
          ...
          className="mt-2 w-full text-xs text-theme-muted hover:text-theme transition-colors py-2"
        >
          Réduire
        </button>
      )}
